...
dashboard/app.py
================
Dashboard interactivo en Streamlit para el Monitor de Calidad del Aire en America Latina.
Incluye KPIs, mapa Folium, resumen LLM, gráficas en pestañas
(serie temporal, predicción 24h, importancia de features, correlación) y
una tabla de alertas.
...
# Colores por categoría para el mapa.
COLOR_CATEGORIA = {
    "Bueno": "green",
    "Moderado": "yellow",
    "Dañino para grupos sensibles": "orange",
    "Dañino": "red",
    "Muy Dañino": "purple",
    "Peligroso": "darkred",
    "Sin datos": "gray",
...
def cargar_datos():
    """
    Carga el dataset procesado desde CSV y fuerza la tipificación numérica.
    """
    ...
    # Sanitización de columnas críticas a numérico para evitar errores de cálculo
...
def ultimas_lecturas(df):
    """Devuelve la última lectura por ciudad (la más reciente)."""
    if df.empty:
        return df
...
st.title("Monitor de Calidad del Aire y Datos Ambientales de América Latina")
st.caption("Universidad Tecnológica de Panamá — Gestión de la Información - Parcial #2")
...
if not df.empty and df["timestamp"].notna().any():
    ...
    fecha_min = fecha_max = _dt.date.today()
...
contaminante_label = st.sidebar.selectbox(
    "Contaminante a graficar", list(CONTAMINANTES.keys())
)
...
if df.empty:
    st.warning(
        "No hay datos procesados todavía. Ejecuta el pipeline con "
        "`python pipeline/actualizar.py --now` o usa el botón "
...
# Fila de KPIs (Corregida contra Nulos y Decimales)
...
# CORRECCIÓN DE SEGURIDAD: Convertimos el promedio a entero redondeado para la función de rangos
...
col1.metric("AQI promedio actual",
            f"{aqi_prom:.0f}" if pd.notna(aqi_prom) else "N/D")

col2.metric("Ciudad más contaminada", ciudad_max,
            f"AQI {aqi_max:.0f}" if pd.notna(aqi_max) and aqi_max > 0 else "N/D")

col3.metric("PM2.5 promedio",
            f"{pm25_prom:.1f}" if pd.notna(pm25_prom) else "N/D")

# Fallback defensivo usando diccionarios .get() para evitar que falte la clave color/etiqueta
col4.metric("Categoría general", categoria_general.get("etiqueta", "Sin datos"))
col4.markdown(
...
# --- Tab 1: serie temporal por ciudad ---
with tab1:
    if contaminante_col in df_filtrado.columns and not df_filtrado[contaminante_col].isna().all():
        fig = px.line(
            ...
            title=f"Evolución de {contaminante_label} por ciudad",
        )
        st.plotly_chart(fig, use_container_width=True)
    else:
        st.info(f"No hay datos de {contaminante_label} disponibles para el rango de fechas.")

# --- Tab 2: predicción 24h ---
with tab2:
    ciudad_pred = st.selectbox(
        "Ciudad para predecir", ciudades_sel or ciudades_disponibles
    )
    with st.spinner(f"Generando predicción para {ciudad_pred}..."):
        pred = entrenar_y_predecir(ciudad_pred)
    ...
    if pred is None or pred.empty:
        ...
        fig_pred.add_trace(go.Scatter(
            x=pred["hora"], y=pred["upper"], mode="lines",
            line=dict(width=0), showlegend=False, name="upper",
        ))
        fig_pred.add_trace(go.Scatter(
            x=pred["hora"], y=pred["lower"], mode="lines",
            fill="tonexty", fillcolor="rgba(0,150,80,0.2)",
            line=dict(width=0), name="Intervalo de confianza",
        ))
        fig_pred.add_trace(go.Scatter(
            x=pred["hora"], y=pred["pm25_predicho"], mode="lines+markers",
            line=dict(color="#00964F", width=2), name="PM2.5 predicho",
        ))
        fig_pred.update_layout(
            title=f"Predicción de PM2.5 — próximas 24h ({ciudad_pred})",
            xaxis_title="Hora", yaxis_title="PM2.5",
        )
        st.plotly_chart(fig_pred, use_container_width=True)

# --- Tab 3: importancia de features ---
with tab3:
    importancias = get_feature_importance()
    if importancias is None or importancias.empty:
        st.info(
            "El modelo clasificador aún no está entrenado o no tiene datos suficientes."
        )
    else:
        fig_imp = px.bar(
...
# --- Tab 4: heatmap de correlación ---
with tab4:
    numericas = df_filtrado.select_dtypes(include="number")
    numericas = numericas.drop(
        columns=[c for c in ["es_fin_de_semana", "mes", "riesgo_salud"]
    ...
    if numericas.shape[1] >= 2:
        ...
        st.info("No hay suficientes variables numéricas para la correlación.")

st.divider()


# Sección de alertas (AQI > 100)
